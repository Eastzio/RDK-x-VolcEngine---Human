...
void AIUI_Node::sendMessage(const std::string& message) {
    ...
    // 使用AI网关发送消息
    ai_gateway_->sendMessage(message, 
        [this](const std::string& response) {
            this->ai_response_callback(response);
        }
    );
}

void AIUI_Node::ai_response_callback(const std::string& response) {
    try {
        string rmText = removeTags(response);
        ...
        std::cerr << "Error processing AI response: " << e.what() << std::endl;
        waiting_for_response_ = false;
    }
}
...
AIUI_Node::AIUI_Node(const std::string &node_name,
    ...
    // 从环境变量获取配置
    const char* api_key_env = std::getenv("VEI_API_KEY");
    if (api_key_env) {
        api_key_ = api_key_env;
    } else {
        api_key_ = "your-default-api-key"; // 或者从配置文件读取
    }
    
    base_url_ = "https://ai-gateway.vei.volces.com/v1";
    
    // 初始化AI网关
    ai_gateway_ = std::make_unique<AIGateway>(base_url_, api_key_);
    
    // 设置系统提示词（可根据机器人特性定制）
    ai_gateway_->setSystemPrompt(
        "你是一个智能机器人助手，请用简洁、友好的语言回答用户问题。"
        "回答要简短明了，适合语音播报。"
    );
    ...
    RCLCPP_INFO(this->get_logger(), "AIUI Node initialized with AI Gateway");
}
