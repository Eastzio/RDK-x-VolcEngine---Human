...
功能: 获取串口返回字段
***************************************/
bool human_mic::UnPackMsgPacket(const string &content, MsgPacket &data)
{
...
功能: 校验串口数据并解析信息
***************************************/
int human_mic::process_data(const unsigned char *buf, int len)
{
	if (buf[2] == 0xff)
	{
		return -1;
...
功能: 过滤数据
***************************************/
int human_mic::uart_analyse(unsigned char buffer)
{
...
功能: 接收下位机发送的信息
***************************************/
bool human_mic::Get_Serial_Data()
{
...
功能: 处理串口异常
***************************************/
void human_mic::handle_serial_error() 
{
...
功能: 串口初始化
***************************************/
void human_mic::initialize_serial() {
    ...
    RCLCPP_ERROR(this->get_logger(),"human_mic can not open serial port,Please check the serial port cable! ");
}
...
功能: 构造函数, 用于初始化
***************************************/
human_mic::human_mic(const std::string &node_name)
...
human_mic::~human_mic()
{
	RCLCPP_INFO(this->get_logger(),"human_mic_node over!\n");
	MicArr_Serial.close();
}
...
int main(int argc,char **argv)
{
	...
	auto mic = std::make_shared<human_mic>("human_mic");
  mic->run();
  rclcpp::shutdown();
	return 0;
}